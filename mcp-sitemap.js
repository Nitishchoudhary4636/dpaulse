...
//       var storedUser = null;
//       try {
//         lastLead = JSON.parse(localStorage.getItem("dpauls_last_lead") || "{}");
//       } catch (e) {}
//       try {
//         storedUser = JSON.parse(localStorage.getItem("dpauls_logged_user") || "{}");
//       } catch (e) {}

//       var phone =
//         (lastLead && lastLead.phone) ||
//         (storedUser && (storedUser.mobile || storedUser.phone)) ||
//         getDataLayerValue(["MCP", "user", "attributes", "mobileNumber"]) ||
//         getDataLayerValue(["MCP", "user", "attributes", "phone"]);

//       var email =
//         (lastLead && lastLead.email) ||
//         (storedUser && storedUser.email) ||
//         getDataLayerValue(["MCP", "user", "attributes", "emailAddress"]) ||
//         getDataLayerValue(["MCP", "user", "attributes", "email"]);

//       var firstName =
//         (lastLead && lastLead.firstName) ||
//         getDataLayerValue(["MCP", "user", "attributes", "firstname"]) ||
//         (storedUser && storedUser.name ? storedUser.name.split(" ")[0] : undefined);

//       var lastName =
//         (lastLead && lastLead.lastName) ||
//         getDataLayerValue(["MCP", "user", "attributes", "lastname"]) ||
//         (storedUser && storedUser.name && storedUser.name.split(" ").length > 1 ? storedUser.name.split(" ").slice(1).join(" ") : undefined);

//       var service =
//         (lastLead && lastLead.service) ||
//         getDataLayerValue(["MCP", "user", "attributes", "Service"]) ||
//         undefined;

//       var travelDate =
//         (lastLead && lastLead.travelDetails) ||
//         getDataLayerValue(["MCP", "user", "attributes", "TravelDate"]) ||
//         undefined;

//       var message =
//         (lastLead && (lastLead.notes || lastLead.requirements)) ||
//         getDataLayerValue(["MCP", "user", "attributes", "Message"]) ||
//         undefined;

//       var userId = phone || email;
...
//     // Expose direct MCP event sender for WhatsApp Lead with form attributes only
//     window.sendMcpWhatsAppLeadEvent = function (leadData) {
//       if (!leadData) return;
//       var phone = leadData.phone || "";
//       var email = leadData.email || "";
//       var firstName = leadData.firstName || "";
//       var lastName = leadData.lastName || "";
//       var service = leadData.service || "";
//       var travelDate = leadData.travelDetails || "";
//       var message = leadData.notes || leadData.requirements || "";
...
//           { name: "global_whatsapp_lead_widget", selector: "#wa-floating-widget" },
...
//             });
//           }),
//           // WhatsApp Floating Lead Form Submission Tracker (capturing form attributes only)
//           SalesforceInteractions.listener("submit", "#wa-lead-form", function () {
//             var firstName = (document.getElementById("wa-first-name") ? document.getElementById("wa-first-name").value : "").trim();
//             var lastName = (document.getElementById("wa-last-name") ? document.getElementById("wa-last-name").value : "").trim();
//             var email = (document.getElementById("wa-email") ? document.getElementById("wa-email").value : "").trim();
//             var phone = (document.getElementById("wa-phone") ? document.getElementById("wa-phone").value.replace(/[^0-9]/g, "") : "").trim();
//             var service = (document.getElementById("wa-service") ? document.getElementById("wa-service").value : "").trim();
//             var travelDate = (document.getElementById("wa-travel-details") ? document.getElementById("wa-travel-details").value : "").trim();
//             var message = (document.getElementById("wa-notes") ? document.getElementById("wa-notes").value : "").trim();
